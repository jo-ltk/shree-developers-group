        ...
        <div className="moving-showcase-track flex min-w-max items-stretch gap-5 pl-5 will-change-transform sm:gap-6 sm:pl-6 lg:gap-7 lg:pl-7">
          {repeatedItems.map((item, index) => (
            <article
              ...
              className="group/slide relative h-[20rem] w-[88vw] shrink-0 overflow-hidden rounded-[1.8rem] bg-black sm:h-[28rem] sm:w-[82vw] lg:h-[40rem] lg:w-[78vw]"
            >
              <Image
                src={item.image}
                alt={item.title}
              ...
              <div className="absolute inset-x-0 bottom-0 px-4 pb-4 sm:px-6 sm:pb-6 lg:px-8 lg:pb-8">
                <div className="moving-showcase-text font-sans text-[clamp(3.4rem,8.8vw,9.2rem)] font-medium uppercase leading-[0.84] tracking-[-0.055em] text-primary-foreground/96 transition-colors duration-300 group-hover:text-[color:color-mix(in_oklab,var(--accent)_28%,white)]">
                  {item.title}
                </div>
              </div>
            </article>
