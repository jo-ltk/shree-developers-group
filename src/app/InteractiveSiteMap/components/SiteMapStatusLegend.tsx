...
export function SiteMapStatusLegend({
  ...
  mode?: "full" | "available-sold" | "sydney-oaks";
}) {
  const items =
    mode === "available-sold" || mode === "sydney-oaks"
      ? LEGEND_ITEMS.filter((item) => item.short === "Sold" || item.short === "Avail" || item.short === "Contract")
      : LEGEND_ITEMS;
    ...
    return (
      ...
      >
        {items.map(({ short, label, fill, stroke }) => {
          const displayLabel =
            (mode === "available-sold" || mode === "sydney-oaks") && short === "Avail"
              ? "Open"
              : short === "Contract"
              ? "Contract"
              : short;
